				...
				<scroll-view :scroll-x="true" class="scroll-view_H">
					<view :class="currentIndex === item.type ?'scroll-view-item_H active':'scroll-view-item_H' "
						...
						<view class="scroll-content">
							...
							<text
								:style="currentIndex == item.type ? 'color:#507ef6' : 'color:#333'">{{item.text}}</text>
						</view>

						<view class="activedicon" v-if="currentIndex === item.type">
							<image src="../../static/images/select.png" mode="" class="imgfilterdark"></image>
						</view>
					</view>
				</scroll-view>
			</view>
			<view class="bankcradlist">
				<view class="titel">
					{{$t('withdraw.receiving.card.number.text')}}
				</view>
				<view class="bankmsg" v-for="(item,index) in typeOptions" :key="item.id" @click="selectCardClick(item)">
					<view class="lefts">
						<image :src="`${wbStr}/${item.img}`" mode=""></image>
						<view class="bankname">
							{{item.label}}
						</view>
					</view>
					<view class="rights">
						<image
							:src="typeValue === item.id ? '../../static/images/rightselect.png':'../../static/images/rightradio.png'"
							mode=""></image>
					</view>
				</view>

				<view class="autotrans" @click="bindCardUsdtClick">
					<image src="../../static/images/add.png" mode=""></image>
					<view class="autofonts">
						{{currentIndex === 1 ? $t('withdraw.add.bank.type.text') : currentIndex === 2 ? $t('withdraw.add.usdt.type.text') : currentIndex === 3 ? $t('backapi.self.withdraw.add.sim.text'):'' }}
					</view>
				</view>
			</view>
			<view class="messagecontents">
				<view class="lists">
					<view class="label">
						{{$t('home.balance')}}
					</view>
					<view class="value hascolor">
						{{allMoneyNum}}
					</view>
				</view>
				<view class="lists">
				...
				<view class="lists">
					...
					<view class="value">
						{{actualAmountTran}} <span class="dymics">({{currencySymbol}})</span>
					</view>
				</view>

			</view>

			<view class="transblock">
				<view class="title">
					{{$t('withdraw.amount.num.text')}}
				</view>
				<view class="content">
					<view class="inputC">
						...
						<input class="t-input" type="number" value="" maxlength=20 :placeholder="inputPlace"
							placeholder-style="font-size:14px;color:rgb(80,103,120);font-weight: 100;"
							v-model="inputWithdrawAmount" @input="rechargeAmountInputChange"
							@focus="rechargeAmountFocus" @blur="rechargeAmountBlur" />
					</view>
					<view class="btnC" @click="allMoneyClick">
				...
				<view class="content">
					<view class="inputC">
						...
						<input class="txmm-input" type="text" value="" maxlength=4 :placeholder="smsCodePlace"
							placeholder-style="font-size:14px;color:#969799" v-model="sms_code"
							@input="rechargeAmountInputChange" />
					</view>
					<view class="btnC" @click="getSmsCodeClick">
						{{defalutCodeTitle}}
					</view>
				</view>
				...
				<view class="content">
					...
					<view class="btnC" @click="forgetfunpass">
			...
			<view class="tab-view">
				...
				<view class="withfraw-btn">
					<view class="tx-active">
						<u-button :ripple="false" :class="changeRechargeBtnStyle" hover-class="none"
							style="overflow: visible;color: #fff;"
							@click="inputWithdrawAmount ? confirmWithdrawClick() : null">
					...
					<view class="info-money">
						<view class="">
							...
							<view class="info-money-num" style="margin-top: 10px;">
								*{{$t('withdraw.desc.list1')}} {{everydayWithdrawFree}}
							</view>
							<view class="info-money-num">
								*{{$t('withdraw.desc.list2')}} {{everydayWithdrawTimes}}
							</view>
							<!-- 	<view class="info-money-num">
								3、{{$t('withdraw.desc.list3')}} {{withdrawalRate}}%
							</view> -->
							<view class="info-money-num">
								*{{$t('withdraw.desc.list4')}} {{withdrawMax}},{{$t('withdraw.desc.list5')}}
								{{withdrawMin}}
							</view>
							<view class="info-money-num">
								*{{$t('withdraw.desc.list6')}} {{withdrawalToday}}
							</view>
							<view class="info-money-num">
								*{{$t('withdraw.desc.list7')}} {{withdrawalRateMin}},{{$t('withdraw.desc.list8')}}
								{{withdrawalRateMax}}
							</view>
							<!-- <view class="info-money-num">
 ...
 -->
		...
		<!-- 		<u-popup v-model="cardsPopBool" mode="bottom" width="315px" border-radius="10">
			<view class="cards-pop">
				...
				<view class="add-card" @click="addNewCardClick">
					<view class="left">
	...
	export default {
		...
		methods: {
			...
			goBackBtnClick() {
				...
				//提现页面返回源页面需清除此状态,否则无法正常显示其默认的内容
