			...
			<view class="">
				<mescroll-body ref="mescrollRef" :fixed="false" top="0" :topbar="false" bottom="100" :bottombar="false"
					...
					<view class="list-item" v-for="(item, index) in dataList" :key="index"
						...
						<view class="one-list">
							<view class="source"
								:style="item.status === 1 ?  'color:#2DB9FF' : item.status === 2 || item.status === 4 ? 'color:#00ff65' : item.status === 3 ? 'color:red':'color:#fff' ">
								<view class="">
									{{item.statusName}}
								</view>
							</view>
						...
						<view class="tit">
							<text class="text">{{$t('match.order.detail.proce.fee.text')}}</text>
							<!-- 用这个是显示提现费率 -->
							<!-- <text class="text content">{{item.rate}}</text> -->
							<!-- 用这个是显示提现手续费 -->
							<text class="text content">{{item.commission / 100}}</text>
						</view>
						...
						<view class="tit notBorder">
							<text class="text">{{$t('user.report.account.change.list.orderno.text')}}</text>
							<text class="text content">{{item.orderNo}}</text>
						</view>
						<view class="tit notBorder" v-if="item.status === 3 ">
							<view class="text">{{$t('withdraw.record.center.show.detail.status.fail.reason.text')}}
							</view>
							<view class="text content" style="width: 50%;word-break: break-word;">{{item.remark}}</view>
						</view>
					</view>
