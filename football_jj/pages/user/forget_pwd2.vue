...
<script>
	...
	export default {
		data() {
			return {
				navTitle: this.$t('backapi.self.edit.change.safe.pwd.usdt'),
				...
				placeholderStyle: "color:#969799;font-size:14px",
				oldPwd: '',
				newPwd: '',
				confirmPwd: '',
		...
		methods: {
			...
			initLang() { //初始化语言
				...
				this.navTitle = this.$t('backapi.self.edit.change.safe.pwd.usdt')
			...
			async updateBtnClick() {
				...
				// }
				let resR = await editPwdPayReq(reqParam, this.localLoginToken)
				if (resR.code === 200) {
