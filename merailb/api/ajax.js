	...
	return new Promise((resolve, reject) => {
		...
		promise.then(response => {
			...
			if (response[0]) { //如果这个为1,可能有错误
				...
				if (resG.code === 2000 || resG.code === 20001 || resG.code ===
					104 || resG.code === 106 || resG.code === 300) { //300是任务需要进行到结算页面
					resolve(response[1].data)
				} else if (resG.code === -105 || resG.code === -
					103) { // -103全局非法请求警告,一般是没有token,-105管理员或者用户的token过期
					let vue = new Vue();
					...
					// reject(resG.msg)
				} else if (resG.code === -101) { //表单提交项验证未通过
					let currData = resG.msg
					let currMsgKey = currData.identification
					...
					// }
					// reject(currData)
				} else if (resG.code === -102) { //-102全局限制警告,
					uni.showToast({
						icon: 'none',
						title: vueInstance.$t(`backapi.faststr`)
					});
					resolve(response[1].data)
					// reject(currData)
				} else if (resG.code === 401) { //401提现金额,

					resolve(response[1].data)
					// reject(currData)
				} else if (resG.code === -888) { //余额不足,支付的时候显示,还需要多少金额
					resolve(response[1].data)
				}
				...
				// } 
				else if (resG.code === 400) { //特定code值,没有绑定银行卡的时候
					resolve(response[1].data)
				} else if (resG.code === 100403) { //出错的响应
					uni.showToast({
						icon: 'none',
