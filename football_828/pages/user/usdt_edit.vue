			...
			<view class="items">
				<view class="uni-list">
					<view class="list-cells money-name">
						...
						<view class="card-right">
							<image src="../../static/images/2.png" mode="" class="icon"></image>
							...
							<view class="selector-item" :style="!showPayTypeBool ? 'display: none;' : ''">
								<scroll-view class="scroll-Y">
									<view class="item active">
										...
										<u-icon name="checkmark" color="#ffb000" size="17px"></u-icon>
									</view>
								</scroll-view>
							</view>
						</view>
				...
				<view class="item">
					...
					<u-input type="text" class="uCard" :placeholder="usdtAddressPlace" :custom-style="customInputStyle"
						maxlength="45" v-model="usdtAddress" @input="inputUsdtChange" :clearable="false" />
				...
				<view class="item">
					...
					<u-input :type="payPassBool ? 'text' : 'password' " :clearable="false" :password-icon="false"
						...
						:placeholder-style="placeholderStyle" maxlength="20" v-model="payPwd"
						@input="inputUsdtChange" />
	...
	export default {
		...
		data() {
			return {
				...
				payPassBool: false,
				navTitle: this.$t('backapi.self.edit.usdt'),
		...
		onShow() {
			...
			eventChannel.on('editUsdtObjs', (data) => {
				this.editData = data
				this.usdtAddress = data.addr
			})
		...
		methods: {
			goBackBtnClick() {
				...
				// const queryAddUsdtUrlFrom = uni.getStorageSync('queryAddUsdtUrlFrom') || 'other'
				// if (!this.addUsdtState_Bool) {
				// 	this.urlFromStr = queryAddUsdtUrlFrom
				// }
			...
			commonRuls() {
				if (this.usdtAddress && this.payPwd && this.smsCode) {
			...
			async usdtAddBtnClick() {
				...
				reqParam.addr = this.usdtAddress
				reqParam.payPwd = this.payPwd
				reqParam.code = this.smsCode
				let resR = await usdtEditReq(reqParam, this.localLoginToken)
				if (resR.code === 200) {
					uni.showToast({
						title: resR.msg,
						icon: 'none',
