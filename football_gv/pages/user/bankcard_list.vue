<template>
	<view>
		...
		<view class="bankCardListView">
			<view class="bankCardCellView">
				<view class="bankCardCellTitle">
					<text>{{$t('user.security.center.bankcard.bank.title.text')}}</text>
				</view>
			...
			<view class="bankCardCellView">
				<view class="bankCardCellTitle">
					<text>{{$t('user.security.center.bankcard.virtual.title.text')}}</text>
				</view>
				<view class="bankCardCellImageAndAdd usdt" :style="usdtStyle">
					<view class="">

					</view>
					<view class="addBtn" @click="addBtnClick(2)">
						+{{$t('user.security.center.bankcard.Add.title.text')}}
					</view>
				</view>
			</view>
		...
		<view class="" style="padding-bottom: 30px;">
			<view class="bankCardListContaines" v-if="card_list.length !== 0">
				<!-- <view class="bankCardListContainesTitle">
					{{$t('user.security.center.bankcard.bank.title.text')}}
				</view> -->
				<view class="bankCardListContainesCell" v-for="(item,index) in card_list" :key="index">
					...
					<view class="editBankBtn" @click="bank_edit_click(item)">
			...
			<view class="bankCardListContaines" v-if="usdt_list.length !== 0">
				...
				<view class="bankCardListContainesCell" v-for="(item,index) in usdt_list" :key="index">
					<view class="bankCardListContainesLeft">
						<view class="bankCardListContainesImage">
							<image :src="usdt_image" mode="" class="bankImage"></image>
						</view>
						<view class="bankCardListContainesCellCenter">
							<view class="bankCardListContainesCellItem">
								{{$t('user.security.center.bankcard.virtual.title.text')}}
					...
					<view class="editBankBtn" @click="usdt_edit_click(item)">
						{{$t('backapi.self.editStr')}}
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			let bankImgStr = '../../static/images/bankCharge.png' + date
		...
		methods: {
			...
			async getUsdtListData() {
				...
				if (resR.code === 200) {
					let resultData = resR.data
					if (resultData) {
						this.usdt_list = resultData
					}
				}
			},
			bank_edit_click(item) {
				uni.navigateTo({
					url: '/pages/user/bankcard_edit',
					success: (res) => {
						// 通过eventChannel向被打开页面传送数据
						res.eventChannel.emit('editBankObjs', item)
					},
					animationType: 'pop-in',
					animationDuration: 200
				});
