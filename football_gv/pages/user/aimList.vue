		...
		<view class="accountListContainer">
			<view class="tableBox">
				<view class="tableBoxCenterTitle">
					...
					<view class="tableBoxCenterTitleItem">
						{{$t('backapi.self.statistics.top.content.aimDays.text')}}
					</view>
				</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				navTitle: this.$t('backapi.self.statistics.top.title.text'),
				searchUsername: '',
				customStyleInput: {
					"border-radius": "5px",
		...
		methods: {
			...
			async getAccountListData(pageNum, pageSize) {
				...
				let resR = await aimListReq(reqParam, this.localLoginToken)
				if (resR.code === 200) {
