		...
		<view class="accountListContainer">
			...
			<view class="tableBox">
				<view class="tableBoxCenterTitle">
					<view class="w45 tableBoxCenterTitleItem">
						{{$t('user.report.account.list.center.list.head.username.text')}}
					</view>
					<view class="w55 tableBoxCenterTitleItem flexRowItem">
						<view class="left">
							{{$t('backapi.self.account.list.data.last.login.text')}}
						</view>
						<view class="right flexColumnItem">
							<image src="../../../static/images/account_up.png" mode="" class="sortImg"
								@click="sortChoose(2)"></image>
							<image src="../../../static/images/account_down.png" mode="" class="sortImg"
								@click="sortChoose(1)"></image>
						</view>
					</view>
				...
				<view class="tableBoxBody">
					...
					<mescroll-uni ref="mescrollRef" :fixed="true" height="900" top="0" :topbar="false" bottom="0"
						...
						<view class="tableBoxBodyTBody" v-for="(item, index) in dataAccountList" :key="item.id">
							<view class="w45 tableBoxBodyTBodyItem tBodyItemUserName">
								{{item.username}}
								<!-- {{item.createdAt | timestampStrSlice}} -->
							</view>
							<view class="w55 tableBoxBodyTBodyItem tBodyItemDate">
							...
							<view class="tableBoxBodyTBodyItem"
								:style="item.status === 1 ? 'color:#00FF65' :'color:#fff'">
...
<script>
	...
	export default {
		mixins: [MescrollMixin, myMixin, accoutListMixin], // 使用mixin
		components: {
		...
		onShow() {
			...
			eventChannel.on('accountListEventClick', async (data) => {
		...
		methods: {
			goBackBtnClick() {
				...
				const queryAccountListUrlFrom = uni.getStorageSync('queryAccountListUrlFrom') || 'other'
				if (!this.accountListState_Bool) {
					this.urlFromStr = queryAccountListUrlFrom
				}
				// console.log("this.urlFromStr", this.urlFromStr);
				if (this.urlFromStr === "report") {
					uni.navigateTo({
