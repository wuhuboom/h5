		...
		<view class="personContainer">
			<view class="personSearchBox">
				...
				<view class="personSearchBox_searchBtn" @click="searchBtnClick">
					{{$t('user.team.report.center.data.query.search.text')}}
				</view>
			</view>
			...
			<view class="personSearchBox_tableBox">
				<view class="personSearchBox_tableBox_hearder">
					...
					<view class="tableBoxHearderWithdraw">
						{{$t('user.team.report.center.list.head.win.lose.text')}}
					</view>
					<view class="tableBoxHearderOperate">
						{{$t('user.team.report.center.list.head.operate.text')}}
					</view>
				</view>
				<view class="personSearchBox_tableBox_tboday">
					<mescroll-uni ref="mescrollRef" :fixed="true" height="900"  top="0" :topbar="false" bottom="0" :bottombar="true" :safearea="false" @init="mescrollInit" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption" >
									
							<view class="personSearchBox_tableBox_tboday_cell" v-for="(item,index) in dataList" :key="index">
								<view class="">
									{{item.date}}
								</view>
								<view class="">
								...
								<view class="personSearchBox_tableBox_tboday_cell_detail" @click="personDetailClick(item)">
									{{$t('user.team.report.center.list.body.detail.text')}}
								</view>
							</view>
		 ...
		 <transition >
			...
			<view class="tableBoxBodyTBodyItem_detail_person" v-if="detailMaskBool">
				<view class="tableBoxBodyTBodyItem_detail_person_tit">
					{{$t('user.team.report.center.detail.data.title.text')}}
				</view>
				<view class="tableBoxBodyTBodyItem_detail_person_content">
					...
					<view class="tableBoxBodyTBodyItem_detail_person_item">
						...
						<view class="tableBoxBodyTBodyItem_detail_person_item_content">
							{{winAmount | moneyFormat }}
						</view>
					</view>
					<view class="tableBoxBodyTBodyItem_detail_person_item">
						<view class="tableBoxBodyTBodyItem_detail_person_item_label">
							{{$t('user.team.report.center.detail.data.activity.text')}}
						</view>
						<view class="tableBoxBodyTBodyItem_detail_person_item_content">
				...
				<view class="tableBoxBodyTBodyItem_detail_order_btn_top">
					<view class="tableBoxBodyTBodyItem_detail_order_btn " @click="closureClick">
						{{$t('user.team.report.center.detail.data.close.text')}}
					</view>
				</view>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				current: 0,
				tabList: [{
					name: this.$t('user.team.report.center.today.text'),
				}, {
					name: this.$t('user.team.report.center.yes.text'),
				}, {
		...
		methods: {
			...
			personDetailClick(item){
				...
				this.betNum = item.betCount
				this.winAmount = item.bingo
				this.activeNum = item.activity
				this.rechargeNum = item.recharge
				this.withdrawNum = item.withdrawal
				this.winLossNum = item.winLose
			},
			detailMaskClick(){
...
.v-enter-active{
  animation: roll-in-left 1s linear;
   -webkit-animation:  roll-in-left 1s linear;
}
.v-leave-active {
	animation: roll-out-right 1s linear;
  -webkit-animation: roll-out-right 1s linear;
}
