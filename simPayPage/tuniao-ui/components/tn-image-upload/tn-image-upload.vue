          ...
          height: $tn.string.getLengthUnitValue(height)
            ...
            borderTopColor: deleteBackgroundColor
              ...
              color: deleteColor
				...
				<!-- 重试按钮 -->
				<view v-if="item.error" class="tn-image-upload__item-preview__error-btn" @tap.stop="retry(index)">Click
					retry</view>
				<!-- 图片信息 -->
	...
	export default {
		...
		methods: {
				...
				chooseFile.then(res => {
					...
					res.tempFiles.map((val, index) => {
						...
						if (val.size > maxSize) {
							...
							this.showToast('Allowed file size exceeded')
						} else {
							if (maxCount <= lists.length) {
								this.$emit('on-exceed', val, lists, this.index)
								this.showToast('Maximum allowed number of files exceeded')
								return
							}
							lists.push({
								url: val.path,
			...
			retry(index) {
				...
				this.$tn.message.loading('Re-upload')
			...
			async uploadFile(index = 0) {
				...
				const task = uni.uploadFile({
					...
					success: res => {
						...
						let data = this.toJson && this.$tn.test.jsonString(res.data) ? JSON.parse(res
							.data) : res.data
			...
			deleteItem(index) {
							...
							if (this.beforeRemove && typeof(this.beforeRemove) === 'function') {
								...
								if (!!beforeResponse && typeof beforeResponse.then === 'function') {
									await beforeResponse.then(res => {
										...
										this.showToast('Removed terminated');
									})
								} else if (beforeResponse === false) {
									// 返回false，终止删除
									this.showToast('Removed terminated');
				...
				// 		// 执行before-remove钩子
				// 		if (this.beforeRemove && typeof(this.beforeRemove) === 'function') {
				// 			let beforeResponse = this.beforeRemove.bind(this.$tn.$parent.call(this))(index, this
				// 				.lists)
				// 			// 判断是否返回promise 
				// 			if (!!beforeResponse && typeof beforeResponse.then === 'function') {
				// 				await beforeResponse.then(res => {
				...
				// 					this.handlerDeleteItem(index)
				// 				}).catch(err => {
				// 					this.showToast('Delete operation was interrupted')
				// 				})
				// 			} else if (beforeResponse === false) {
				// 				this.showToast('Delete operation was interrupted')
				// 			} else {
			...
			doPreviewImage(url, index) {
				...
				uni.previewImage({
					...
					fail: () => {
						this.showToast('Failed to preview image')
					}
				})
			},
			// 检查文件后缀是否合法
			checkFileExt(file) {
				...
				if (!noArrowExt) this.showToast(`not support${fileExt}format file`)
				return noArrowExt
			}
		}
	}
